...
//        @salesforce/cli/2.108.6 darwin-arm64 node-v22.19.0
        ...
        if status == 0, let rawOutput = output {
            // The regex captures the version number in the first capturing group.
            // Example: "@salesforce/cli/2.108.6" -> match.output.1 is "2.108.6"
            let regex = /cli\/((?:\.?[0-9]+)+)/
            if let match = rawOutput.firstMatch(of: regex) {
                // Return the content of the first capturing group as a String.
                print("Current verion: \(String(match.output.1))")

                return String(match.output.1)
            } else {
                print("No se encontró la versión en la salida de CLI: \(rawOutput)")
            }
        } else {
            print("El comando 'sf --version' falló o no produjo una salida válida.")
            if let outputError = output {
                print("Salida de error: \(outputError)")
            }
        }
